...
async function refetchTwitterVideos() {
  ...
  const uniqueCreatorIds = [
    ...new Set(twitterCreators?.map((tc) => tc.creator_id) || []),
  ];
  const twitterCreatorsList = creators.filter((c) =>
    uniqueCreatorIds.includes(c.id)
  );

  console.log(
    `Found ${twitterCreatorsList.length} Twitter creators to check for videos\n`
  );
  ...
  for (const creator of twitterCreatorsList) {
    ...
    try {
      const results = await fetcher.fetchTwitterContent([searchTerm], {
        maxTweets: 5,
      });

      const withVideos = results.filter((r) =>
        r.media_urls?.some(
          (m) => m.type === 'video' && m.url?.includes('video.twimg.com')
        )
      );
