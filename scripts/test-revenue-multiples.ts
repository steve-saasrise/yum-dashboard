...
async function testRevenueMultiples() {
  ...
  if (profile) {
    ...
    console.log(`Market Cap (from profile): $${(profile.marketCapitalization / 1000).toFixed(1)}B`);
    console.log(`Shares Outstanding: ${(profile.shareOutstanding / 1000).toFixed(1)}B shares`);
  }

  if (financials?.metric) {
    console.log(`\nFinancial Metrics:`);
    console.log(`Revenue per Share (TTM): $${financials.metric.revenuePerShareTTM || 'N/A'}`);

    if (financials.metric.revenuePerShareTTM && profile?.shareOutstanding) {
      // Calculate total revenue
      const totalRevenueMillion = (financials.metric.revenuePerShareTTM * profile.shareOutstanding) / 1000000;
      console.log(`\nCalculated Total Revenue: $${(totalRevenueMillion / 1000).toFixed(1)}B`);

      // Calculate revenue multiple
      const marketCapMillion = profile.marketCapitalization || (quote!.c * profile.shareOutstanding) / 1000000;
      const revMultiple = marketCapMillion / totalRevenueMillion;

      console.log(`Market Cap (millions): $${marketCapMillion.toFixed(0)}M`);
      console.log(`Total Revenue (millions): $${totalRevenueMillion.toFixed(0)}M`);
      console.log(`Revenue Multiple: ${revMultiple.toFixed(1)}x`);

      console.log(`\nFormula Check:`);
      console.log(`${marketCapMillion.toFixed(0)} ÷ ${totalRevenueMillion.toFixed(0)} = ${revMultiple.toFixed(1)}x`);
    }
  }
  ...
  console.log('You can verify at: https://www.google.com/finance/quote/CRM:NYSE');
}
