...
async function testTwitterVideo() {
  ...
  try {
    ...
    const tweetsWithVideos = items.filter((tweet: any) => {
      return tweet.extendedEntities?.media?.some((m: any) => 
    ...
    if (tweetsWithVideos.length > 0) {
      ...
      console.log('Tweet text:', firstVideoTweet.text?.substring(0, 100) + '...');
      ...
      if (items.length > 0) {
        console.log('Tweet:', JSON.stringify(items[0], null, 2).substring(0, 1000) + '...');
      }
    }
