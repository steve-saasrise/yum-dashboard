...
async function testUrlOperations() {
  ...
  console.log('1. Open the browser developer tools and copy your session cookie');
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/creators/${CREATOR_ID}/urls`, {
      headers
    });
  ...
  try {
    ...
    if (data.data?.id) {
      ...
      const updateResponse = await fetch(`${BASE_URL}/api/creators/${CREATOR_ID}/urls/${data.data.id}`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({
      ...
      const dupResponse = await fetch(`${BASE_URL}/api/creators/${CREATOR_ID}/urls`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
      ...
      const deleteResponse = await fetch(`${BASE_URL}/api/creators/${CREATOR_ID}/urls/${data.data.id}`, {
        method: 'DELETE',
        headers
      });
  ...
  console.log('Try to delete all URLs from a creator - the last one should be protected');
  console.log('---\n');
